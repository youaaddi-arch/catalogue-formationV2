...
def extraire_date_depuis_nom(nom_fichier: str) -> Optional[str]:
    """
    Tente d'extraire une date depuis le nom d'un fichier CERFA.
    Supporte les formats : dd/mm/yyyy, dd-mm-yyyy, dd.mm.yyyy, yyyymmdd, dd_mm_yyyy.
    """
    patterns = [
    ...
    for piece_config in toutes_pieces:
        ...
        for fichier in fichiers_dossier:
            ...
            if piece_config["id"] in ids_pieces:
                ...
                # Si c'est un CERFA et que l'apprenti n'a pas de date d'embauche,
                # tenter de l'extraire depuis le nom du fichier
                if piece_config["id"] == "cerfa" and not apprenti.date_embauche:
                    date_extraite = extraire_date_depuis_nom(fichier["name"])
                    if date_extraite:
                        apprenti.date_embauche = date_extraite
                        logger.info(
                            f"  Date d'embauche extraite du CERFA: {date_extraite}"
                        )
                break
